...
import type { LiveChatMessageListRequest } from './gen/stream_list.js'
import {
...
export class YouTubeAdapter extends BasePlatformAdapter {
  ...
  private readonly MAX_RECONNECT_DELAY = 60_000 // Max 60 seconds
  ...
  async connect(channelIdOrHandle: string): Promise<void> {
    ...
    if (tokens.expiresAt && tokens.expiresAt < now + 300) {
      ...
      try {
        ...
        log.error('[YouTube] Failed to refresh token', { error: String(error) })
        this.emit('status', {
  ...
  private async tryFetchLiveChatAndConnect(): Promise<void> {
    ...
    try {
      ...
      const isNoLiveBroadcast =
        errorMessage.includes('No active live broadcast') ||
  ...
  private readonly MAX_LIVECHAT_RETRY_DELAY = 300_000 // Max 5 minutes — search costs 100 quota units
  private readonly BASE_LIVECHAT_RETRY_DELAY = 10_000 // Start with 10 seconds
  ...
  private scheduleLiveChatRetry(): void {
    ...
    const baseDelay = Math.min(
      this.BASE_LIVECHAT_RETRY_DELAY * 2 ** this.liveChatRetryAttempts,
  ...
  private async fetchLiveChatId(channelOrVideoId: string): Promise<string> {
    ...
    log.info(`[YouTube] Looking for live chat, input="${channelOrVideoId}", clean="${cleanInput}"`)
  ...
  private async resolveChannelIdViaBackend(handle: string): Promise<string | null> {
    try {
      ...
      log.warn(`[YouTube] Backend resolve request failed`, { error: String(error) })
    }
    return null
  }
  ...
  private startGrpcStream(): void {
    if (!this.liveChatId || !this.accessToken || this.isPolling) {
      return
    }
  ...
  private async pollMessages(): Promise<void> {
    ...
    if (!this.grpcClient) {
      ...
      this.grpcClient = new V3DataLiveChatMessageServiceClient(YOUTUBE_GRPC_ENDPOINT, sslCreds, {
    ...
    try {
      ...
      log.error('[YouTube] gRPC stream error', {
    ...
    switch (type) {
      case LiveChatMessageSnippet_TypeWrapper_Type.TEXT_MESSAGE_EVENT: {
        ...
        const normalized: NormalizedChatMessage = {
          author: { avatarUrl, badges, displayName, id: authorId },
      ...
      case LiveChatMessageSnippet_TypeWrapper_Type.SUPER_CHAT_EVENT: {
        ...
        const event: NormalizedEvent = {
          ...
          timestamp,
          type: 'superchat',
          user: { avatarUrl, displayName, id: authorId },
        }
      ...
      case LiveChatMessageSnippet_TypeWrapper_Type.NEW_SPONSOR_EVENT: {
        ...
        const event: NormalizedEvent = {
          data: { isUpgrade: ns?.isUpgrade, levelName: ns?.memberLevelName },
  ...
  private scheduleReconnect(): void {
    if (!this.shouldReconnect || this.isPolling) {
      return
    }
    ...
    // Calculate exponential backoff with jitter
    const baseDelay = Math.min(
      this.BASE_RECONNECT_DELAY * 2 ** this.reconnectAttempts,
