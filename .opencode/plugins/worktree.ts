...
import type { ActiveLaunchContext } from './worktree/launch-context'
import {
  addSession,
  clearPendingDelete,
  getPendingDelete,
...
import type { TerminalResult } from './worktree/terminal'
...
async function forkWithContext(
  ...
  try {
    ...
    throw new WorktreeError('Failed to get root session ID', 'forkWithContext', error)
  }
  ...
  try {
    ...
    const destWorkspaceDir = path.join(workspaceBase, projectId, forkedSession.id)
    const destDelegationsDir = path.join(delegationsBase, projectId, forkedSession.id)
    await rm(destWorkspaceDir, { force: true, recursive: true }).catch((error) => {
      client.app
        .log({
          body: {
            service: 'worktree',
            level: 'error',
            message: `forkWithContext: Failed to clean up workspace dir ${destWorkspaceDir}: ${error}`,
          },
        })
        .catch(() => {})
    })
    await rm(destDelegationsDir, { force: true, recursive: true }).catch((error) => {
      client.app
        .log({
          body: {
            service: 'worktree',
            level: 'error',
            message: `forkWithContext: Failed to clean up delegations dir ${destDelegationsDir}: ${error}`,
          },
        })
        .catch(() => {})
    })
    await client.session.delete({ path: { id: forkedSession.id } }).catch((error) => {
      client.app
        .log({
          body: {
            service: 'worktree',
            level: 'error',
            message: `forkWithContext: Failed to clean up forked session ${forkedSession.id}: ${error}`,
          },
        })
        .catch(() => {})
    })
...
async function copyFiles(
  ...
  for (const file of files) {
    if (!isPathSafe(file, sourceDir, log)) {
      continue
    }
