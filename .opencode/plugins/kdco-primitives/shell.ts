...
export function escapeBash(str: string): string {
  ...
  return str
    .replace(/\\/g, String.raw`\\`) // Backslash first (order matters!)
    .replace(/"/g, String.raw`\"`) // Double quotes
    .replace(/\$/g, String.raw`\$`) // Dollar sign (variable expansion)
    .replace(/`/g, '\\`') // Backticks (command substitution)
    .replace(/!/g, String.raw`\!`) // History expansion
    .replace(/\n/g, ' ') // Newlines -> spaces
